...
The world is defined using a .dot file (locations, paths, entities) and an actions.xml file (verbs, interactions, and effects).
...
Each action is driven by XML-defined rules.
For example:
...
This structure makes the game world fully extensible — new locations and interactions can be added without touching the core Java code.
...
You’ll need curiosity, observation, and a bit of experimentation to reach it.
...
✨ Notes
	•	Each game session is interactive and persistent across connected local clients.
	•	The modular design allows you to build your own worlds and define new actions.
	•	The engine is deliberately minimal — designed to be extended and explored.
